...
import { marketsStorage, MarketEvent } from "@/lib/storage";
...
export default function EditMarketScreen() {
    ...
    const [quickItems, setQuickItems] = useState<{ name: string; priceText: string }[]>([]);
    const [loading, setLoading] = useState(true);

    const loadMarket = useCallback(async () => {
        const all = await marketsStorage.getAll();
        const found = all.find(m => m.id === id);
        if (found) {
            setName(found.name);
            setLocation(found.location);
            setDate(found.date);
            setStandFee(found.standFee.toString().replace('.', ','));
            setTravelCost(found.travelCost.toString().replace('.', ','));
            setNotes(found.notes);

            const qItems = found.quickItems?.map(i => ({
                name: i.name,
                priceText: i.price.toString().replace('.', ',')
            })) || [{ name: "", priceText: "" }];

            if (qItems.length === 0) qItems.push({ name: "", priceText: "" });

            setQuickItems(qItems);
        } else {
            Alert.alert("Fehler", "Markt nicht gefunden");
            router.back();
        }
        setLoading(false);
    }, [id]);
    ...
    const saveMarket = async () => {
        ...
        await marketsStorage.update(id!, {
            name: name.trim(),
            date,
